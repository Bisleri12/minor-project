...
def text_steno():
    ...
    def decode():
        ...
        def browse():
            """Opens a prompt for selecting files"""
            global file_loc
            file_loc = askopenfilename(parent=dec, initialdir=os.getcwd(), title='Select File to DECODE',filetypes=[('Text files', '.txt')], defaultextension='.txt')
        ...
        def work(event=None):
            ...
            try:
                ...
                show_bu = Button(text_win, text='Exit', bg='yellow', fg='red',command=text_win.destroy, font=cas)
